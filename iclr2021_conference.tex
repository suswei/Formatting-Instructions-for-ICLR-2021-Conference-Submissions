...
\label{def:RLCT}
\end{definition}
In regular statistical models, the RLCT is equal to $d/2$, while in strictly singular models, the RLCT is bounded above by $d/2$ if realizability is satisfied  {\citep[Theorem 6.4]{watanabe_algebraic_2009}}
...
\]
where $A_i: \mathbb{R}^{d_i} \lto \mathbb{R}^{d_{i+1}}$ are affine functions, $d_1 = N, d_{k+}+ M$ and for $1 \le i \le k-1$ the function $\operatorname{ReLU}_i: \mathbb{R}^{d_{i+1}} \lto \mathbb{R}^{d_{i+1}}$ is coordinate-wise $\operatorname{ReLU}$. Let $W$ be a compact subspace of the space $\mathbb{R}^D$ be the space of affine functions $(A_i)_{i=1}^k$ with coordinates $w_i$, so that $f$ may be viewed as a function $f: \mathbb{R}^N \times W \lto \mathbb{R}^M$. We define
\[
...
\]
Then it is a standard calculation that
\[
...
\]
where $\langle -, - \rangle$ denotes the dot product on $\mathbb{R}^M$.
...
\begin{lemma} For any hidden node in the network with coordinates $(w_k)_k$ for the incoming weights, $(q_i)_i$ for the outgoing weights and $b$ for the bias the function $f$ is a solution of the following differential equation
\begin{align*}
\Big\{ \sum_k w_{k}\frac{\partial}{\partial w_k} + b \frac{\partial}{\partial b}-\sum_{i} q_i\frac{\partial}{\partial q_i} \Big\} f = 0.
\end{align*}
\end{lemma}
\begin{proof}
...
Input and target variables are both in $\mathbb R^3$. The distribution of $x$ is set to $q(x)=$ ???. We simulate the data under the realizability assumption, i.e. we draw $y$ given $x$ according to \eqref{eq:genexp_model} for some $w_0 \in W$, drawn randomly. 
...
$$
We use HMC (NUTS variant) to draw $w_1,\ldots,w_R$ from $\tilde{\mathcal D_n}$. Then we approximate $\textbackslash\{\}tilde p(y|x, \textbackslash\{\}mathcal D\_n)$ with
\[

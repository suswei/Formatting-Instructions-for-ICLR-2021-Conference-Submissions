...
\end{equation}
Then it is a standard calculation that the associated Fisher information matrix is
...
\end{equation}
As long as the prior $\varphi(w)$ is non-zero on $W_0$ it does not affect the relevant features of the volume, so we may assume $\varphi$ is constant on the region of integration in the first $d'$ directions and normal in the remaining directions, so up to a constant depending only on $d'$ we have
...
\ee
for any $a > 0$, $a \neq 1$. The right hand side of (\ref{eq:limit_volumedim}) is the \emph{volume dimension} associated to $K$ and $w_0$. It can be easily shown in this special case that the RLCT $\lambda = d'/2$. In other words, $2\lambda$ is the effective number of parameters in the model.
...
We derived $V(t)$ in \eqref{eq:volume_singular} under the special case when the KL divergence can be written as in \eqref{eq:local_Kw}.
%With this in mind the conceptual link between volume dimension and generalization error can be seen as follows. Singular learning theory bears out a general connection between $\Delta E / \Delta T$, the Bayesian generalization error and the volume dimension. 
More generally, for any singular model satisfying some natural conditions \citet[Theorem 7.1]{watanabe_algebraic_2009} shows that
\[
V(t) = c t^\lambda (- \log t)^{m-1} + o( t^\lambda ( - \log t)^{m-1})
\]
where $\lambda$ is the RLCT. It follows that $\lambda$ is the volume dimension computed by (\ref{eq:volume_singular}).

\begin{remark}
It is folklore in the deep learning community that flatter minima have better generalization properties \citep{hinton_keeping_1993, hochreiter1997flat} and this claim has been revisited in recent years \citep{chaudhari2019entropy, smith2017bayesian, jastrzkebski2017three, Zhang:2018MolPh.116.3214Z}. 
...
\]
where the $t^{d'/2}$ should be thought of as the volume of the enclosing ball (i.e. think of cutting out all ``almost true'' parameters within an epsilon ball of some radius around $w_0$). You can get rid of the constant terms by defining your denominator in the volume, so that this calculates the entropy to be a sum of the $\log \lambda_i$. However this doesn't get the correct behavior of the generalization error. This corresponds to an error analysis that considers uncorrelated errors $\Delta w_i$ of the same size.
...
While in a regular statistical model, the three estimators Bayes predictive distribution, MAP, and MLE have the \textit{same} leading term in their asymptotic generalization behavior, the same is not true in singular models.
...
We set $q(x) = N(0,I_3)$. 
For the realizable case, $q(y|x) = p(y|x,\theta_0)$ where $\theta_0$ is drawn randomly according to the default initialization in PyTorch when model \ref{eq:genexp_model} is instantiated. How to use multiple data
sets $\mathcal D_n = \{(x_i,y_i)\}_{i=1}^n$, $(x,y) \sim q(x,y) = q(y|x)q(x)$ and a large testing set drawn also from $q(x,y)$ to assess $E_n G(n)$ is discussed in Appendix \ref{appendix:generalizaton}. 

 Since $f$ is a hierarchical model, let's write it as $f_\theta(\cdot) = h(g(\cdot;v);w)$ with the dimension of $w$ being relatively small. Let $\theta_{map} = (v_{map}, w_{map})$ be the MAP estimate for $\theta$ in model \ref{eq:genexp_model}. The idea of our simple approximate Bayesian scheme is to freeze the network weights at the MAP estimate for early layers and perform approximate Bayesian inference for the final layers, e.g., freeze the parameters of $g$ at $v_{map}$ and perform MCMC over $w$. 
...
In Appendix \ref{appendix:generalizaton}, we also present results analogous to Figure ??? for several variations on the data-generating mechanism are discussed, including violations of realizability. ???summarize the lesson???
...
Consider again the Gaussian model $p(y|x,w)$ in \eqref{eq:gaussian_model_in_w} where this time $f_\theta: \mathbb{R}^2 \longrightarrow \mathbb{R}$ is a two-layer ReLU network:
\begin{gather*}
f_\theta(x) = c + \sum_{i=1}^H q_i \operatorname{ReLU}( \langle w_i, x \rangle + b_i )
\end{gather*}
where $\theta = (\{w_i\}_{i=1}^H, \{b_i\}_{i=1}^H, \{q_i\}_{i=1}^H, c) \in \mathbb{R}^{4H+1}$ and $w_i \in \mathbb{R}^2, b_i \in \mathbb{R}, q_i \in \mathbb{R}$ for $1 \le i \le H$. We let $W$ be some compact neighborhood of the origin in $\mathbb{R}^{3H+1}$ which contains all the networks that we are about to define. 
%\begin{equation}
...
The expected Bayesian generalization error can be connected to the Bayes free energy via the following relation:
\[
{\E}_n G(n) = \E F(n+1) - \E(F_n)
\]
Then we have the expected Bayes generalization error is related to the RLCT as follows
\begin{equation}
{\E}_n G(n) = \lambda/n + o(1/n)
...
\textbf{MAP training}
The MAP estimator is found via stochastic gradient descent on the MSE loss with minibatch set to ???. Training was set to 5000 epochs. Early stopping was not employed.
...
\textbf{Laplace in the last layer(s)}
Describe Laplace in last layer and reduced regression layers.


\textbf{MCMC in the last layer(s)}
 Let $\tilde x_i = g_{v_{map}}(x_i)$. Define a new transformed dataset $\tilde{\mathcal D}_n = \{(\tilde x_i, y_i) \}_{i=1}^n$. We perform MCMC to sample the posterior over $w$:
$$
p(w | \tilde{\mathcal D}_n) \propto p(\tilde{\mathcal D}_n | w) \varphi(w) = \Pi_{i=1}^n \exp\{-|| y_i - h_w \circ g_{v_{map}}(x_i) ||^2/2\} \varphi(w)
$$
...
$$
We use HMC (NUTS variant) \citep{hoffman2014no} to draw $w_1,\ldots,w_R$ from $\tilde{\mathcal D}_n$. Then we approximate $\tilde p(y|x, \mathcal D_n)$ with
\[

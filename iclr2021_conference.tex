...
For a (model, truth, prior) triplet $(p(y|x,w),q(y|x),\varphi)$, let $-\lambda$ be the maximum pole of the corresponding zeta function. We call $\lambda$ the \textbf{real log canonical threshold} (RLCT) of the (model, truth, prior) triplet. The RLCT is the central quantity of singular learning theory. 
%\label{def:RLCT}
%\end{definition}
...
\end{figure}
 ...
 Since $f$ is a hierarchical model, let's write it as $f_\theta(\cdot) = h(g(\cdot;v);w)$ with the dimension of $w$ being relatively small. Let $\theta_{map} = (v_{map}, w_{map})$ be the MAP estimate for $\theta$ in model \ref{eq:genexp_model} using batch gradient descent. The idea of our simple approximate Bayesian scheme is to freeze the network weights at the MAP estimate for early layers and perform approximate Bayesian inference for the final layers\footnote{This is similar in spirit to \citet{kristiadi_being_2020} who advocate "being Bayesian" a little bit fixes overconfidence. This is acheived by using the Laplace approximation for the final layer of a ReLU network, an approach we did not find to work well in our experiments. It is also worth noting that \citet{kristiadi_being_2020} do not attempt to formalize what it means to "fix overconfidence"; the precise statement should be in terms of $G(n)$.}. e.g., freeze the parameters of $g$ at $v_{map}$ and perform MCMC over $w$. 
...
Deep neural networks are singular statistical models, and that's good: the presence of singularities is \emph{necessary} for overparametrized neural networks to have low generalization error. As it stands, singular learning theory is a promising foundation for a mathematical theory of deep learning, but much remains to be done. From our point of view the most pressing open problems are:
...
$$
It follows that $\{ K = 0 \}$ is globally a graph over $GL(H;
\mathbb{R})$.  Indeed, the set $(B^{-1}B_0 A_0, B)$ with $B \in GL(H;
\mathbb{R})$ is exactly $\{ K = 0 \}$.  Thus $\{ K = 0\}$ is a smooth
$H^2$-dimensional submanifold of $\mathbb{R}^{H^2} \times
\mathbb{R}^{H \times M}$.  

To prove that $\{ K = 0 \}$ is minimally singular it suffices to show that
%\begin{equation}
%\label{eq:1}
$$
...
\overline{B}) \in \{ K = 0 \}$ for which $\{ v = 0 \} = \{ K = 0 \}$

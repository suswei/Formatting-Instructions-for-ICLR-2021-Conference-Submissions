...
For instance, over-parameterised neural networks can easily fit random labels \citep{zhang2016understanding,du2018gradient,allen2019convergence} indicating that complexity measures such as Rademacher complexity is very large.
...
$
where $\lambda$ is the local RLCT and $c$ is a constant.
We are assuming that in a sufficiently small neighborhood of $v_0$ the point RLCT at $v_0$ \citep[Definition 2.7]{watanabe_algebraic_2009} is less than or equal to the RLCT at every point in the neighborhood so that the multiplicity $m = 1$, see Section 7.6 of \citep{watanabe_algebraic_2009} for relevant discussion.

...
Following the conventions in \p{watanabe_algebraic_2009}, we have the following bifurcation of statistical models:
\begin{definition}
...
The generalization puzzle is one of the central mysteries of deep learning. Theoretical investigations into the matter is an active area of research \citep{neyshabur_exploring_2017}. Some capacity measures are based on workarounds concerning the degenerate Hessian, e.g., \citet{thomas_information_2019}. ???Briefly review following works \citep{maddox_rethinking_2020}, \citep{gao_degrees_2016}, \citep{sun_lightlike_2020}.???


Far more precise statements regarding the generalization behavior in singular models can be made using singular learning theory.
...
Recall in the Bayesian paradigm, prediction proceeds via the so-called Bayes predictive distribution,
%\begin{equation}
$
...
By {\citet[Theorem 1.2 and Theorem 7.2]{watanabe_algebraic_2009}}, we have\footnote{ In contrast, \citet{zhang_energyentropy_2018} and  \citet{le_bayesian_2018} rely on the Laplace approximation to explain the generalization of the Bayes predictive distribution though both works acknowledge the Laplace approximate is inappropriate. }
\begin{equation}
{\E}_n G(n) = \lambda/n + o(1/n)  \text{ if $\hat q_n$ is the Bayes predictive distribution,}
\label{eq:Bayes_generalization}
\end{equation}
where $\lambda$ is the RLCT corresponding to the triplet $( p(y|x,\theta), q(y|x), \varphi(\theta) )$. A quick sketch of the derivation of \eqref{eq:Bayes_generalization} is provided in Appendix \ref{appendix:generalization_theory}.
...
\end{figure}
 ...
 Since $f$ is a hierarchical model, let's write it as $f_\theta(\cdot) = h(g(\cdot;v);w)$ with the dimension of $w$ being relatively small. Let $\theta_{map} = (v_{map}, w_{map})$ be the MAP estimate for $\theta$ in model \ref{eq:genexp_model} using batch gradient descent. The idea of our simple approximate Bayesian scheme is to freeze the network weights at the MAP estimate for early layers and perform approximate Bayesian inference for the final layers\footnote{ \citet{kristiadi_being_2020} use a Laplace approximation for the final layer of a ReLU network, an approach we did not find to work well in our experiments. It is also worth noting that the title of \citet{kristiadi_being_2020} claims that being Bayesian fixes overconfidence although this is never formalized in terms of $G(n)$.}. e.g., freeze the parameters of $g$ at $v_{map}$ and perform MCMC over $w$. 
...
\begin{table}[h]
	\centering
	\caption{\footnotesize RLCT estimates for ReLU and SiLU networks. We observe the RLCT increasing as $m$ increase, i.e., the true distribution becomes more ``complicated'' relative to the supposed model.}
	\label{table:hyper}
    \begin{tiny}
    \begin{tabular}
    {r l l l l}
...
Deep neural networks are singular statistical models, and that's good: the presence of singularities is \emph{necessary} for overparametrised neural networks to have low generalisation error. As it stands, singular learning theory is a promising foundation for a mathematical theory of deep learning, but much remains to be done. From our point of view the most outstanding open problems are:

\textbf{Real-world datasets are unrealisable.} Among the most profound experimental results in deep learning are the existence of power laws in language model training \citep{??} and this is an indication of interesting new phenomenon in singular learning theory in the case where the true distribution is unrealisable. However, so far this case has been little studied.

\textbf{SGD vs the posterior.} A growing number of works \citep{Simsekli17,mandt_stochastic_2018,smith_stochastic_2018} suggest that some version of mini-batch SGD may be governed by SDEs that have the posterior distribution as its stationary distribution. If this is indeed true, then combined with our knowledge that the Bayes predictive distribution is superior to MAP and MLE, we could use SGD to sample from the posterior and form an approximate Bayes predictive distribution. However from a theoretical perspective this remains unclear.

\textbf{RLCT estimation for large networks.} Theoretical RLCTs are not known for modern neural networks where the weights number on the order of $n$. Theoretical RLCTs for neural networks have been cataloged for very few singular models at significant effort\footnote{Hironaka's resolution of singularities guarantees existence. However it is difficult to do the required blowup transformations in high dimensions to obtain the standard form.}, namely single-hidden-layer $\tanh$ network \citep{aoyagi_resolution_2006} and reduced rank regression \citep{aoyagi_stochastic_2005}. Further, no method is known for estimating RLCTs of large neural networks. In theory MCMC provides the gold standard but this approach does not scale to large networks. We made a (unsuccessful) foray into variational inference for sampling from the Bayesian posterior but this did not prove sufficient for accurate RLCT estimation. We believe RLCT estimation is a challenging benchmark task for any method that purports to approximate the Bayesian posterior of a neural network.

The intractability of RLCT estimation for large neural networks may not in reality be a significant deterrent to reaping the insights offered by singular learning theory. For instance, used in the context of model selection, the exact value of the RLCT is not as important as model selection consistency, i.e., if using the estimated RLCT leads to the same model being selected as if the true RLCT were being used. We also demonstrated the utility of singular learning results such as \eqref{eq:Bayes_generalization} and \eqref{eq:map_generalization} which can be exploited even without knowledge of the exact value of the RLCT.
...
Assuming the conditions of Theorem 4 in \citep{watanabe_widely_2013} hold, we have
\begin{equation}

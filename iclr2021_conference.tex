...
To understand why classical measures of capacity fail to say anything meaningful about deep neural networks (DNN), it's important to distinguish between two different types of statistical models. Suppose we are interested in estimating the true (unknown) conditional distribution $q(y|x)$ with a class of models $\{p(y|x,w): w \in W\}$. Let $W_0 = \{w \in W: p(y|x,w)=q(y|x))\}$ be the set of true parameters.
...
To a model-truth-prior triplet, we define the zeta function of a statistical model as
\begin{equation}
\zeta(z) = \int K(w)^z \varphi(w) \,dw, \quad z \in \mathbb C,
\end{equation} 
where $\varphi(w)$ is a prior on the network weights $w$ and $K(w)$ is the Kullback-Leibler divergence between the model $p(y|x,w)$ and the true distribution $q(y|x)$:
\begin{equation}
    K(w) := \int \!\int q(y|x) \log \frac{ q(y|x) }{ p(y|x,w) } q(x) \,dx \,dy.
\end{equation}
To every such triplet of model-truth-prior, we have the following object.
...
One of the most accessible results in singular learning theory is the work related to WBIC. We explain briefly the developments here since many in the deep learning community may not be aware of this work.
...
Suppose we have a dataset of input-target pairs $\mathcal D_n = \{(x_i,y_i)\}_{i=1}^n$ drawn from the joint distribution $q(x,y) = q(y|x) q(x)$ with both $q(y|x)$ and $q(x)$ unknown. Let $\{p(y|x,w): w \in W\}$ be a class of models. Let $W_0 = \{w \in W: p(y|x,w)=q(y|x))\}$ be the set of true parameters.

\begin{definition}
We say $q(y|x)$ is \textbf{realisable} by the model $p(y|x,w)$ if $W_0$ is non-empty.
\end{definition}
This assumption will be critical to the framework we develop here. Modifications to the theory are needed in the case that $q(y|x)$ is not realisable.
...
$$
To account for sampling variability, we will work with the \textit{average generalization error}, $E_n G(n)$, where $E_n$ denotes expectation over the dataset $\mathcal D_n$.
If $\hat q_n$ is the Bayes predictive distribution, then 
\begin{equation}
E_n G(n) = \lambda/n + o(1/n)
\label{eq:RLCT_generalization}
\end{equation}
where $\lambda$ is the RLCT corresponding to the triplet $( q(y|x), p(y|x,\theta), \varphi(\theta) )$. We provide a quick sketch of the derivation of \eqref{eq:RLCT_generalization} in Appendix ???.
...
\end{equation}
where $f_\theta: X \to Y$ is a neural network. Since $f$ is a hierarchical model, let's write it as $f_\theta(\cdot) = h(g(\cdot,v);w)$ with the dimension of $w$ being relatively small, e.g., 20 or so. Let $\theta_{map} = (v_{map}, w_{map})$ be the MAP estimate for $\theta$ in model \ref{eq:genexp_model}. The idea of our simple approximate Bayesian scheme is to freeze the network weights for $g$ at the MAP estimate $v_{map}$ and perform MCMC over $w$. 

Throughout the experiments, the initial layers of $f$, i.e., the architecture of $g$, is a feedforward ReLU block. The network $h$ will be either two linear layers with the identity activation function or the ReLU activation function. The architecture of $f$ used in the experiments is described in further detail in Appendix ???.
...
\label{eq:last_layer}
\end{equation}
Details of these approximate Bayesian schemes are found in Appendix ???.

From the outset, we expect the approximate Bayes predictive distribution resulting from the laplace approximation for \eqref{last_two_layers} to be inadequate since the model over $w$ is singular. We do however expect the last-layer-only laplace approximation for \eqref{last_layer} to be sound, albeit poor performing, since the model is now regular. Next, we expect the Bayes predictive distribution based on the MCMC approximation of \eqref{last_two_layers} and \eqref{eq:last_layer} to be both superior to the laplace approximations and the MAP. We further expect the predictive distribution based on the the MCMC approximation in the last two layers (\eqref{last_two_layers}) to have better generalization error than MCMC approximation in the last layer only (\eqref{eq:last_layer}) since the former is closer to the Bayes predictive distribution.
...
\end{equation}
where $\beta_0$ is a positive constant and $U_n$ is a sequence of random variables satisfying $\E_{\mathcal D_n} U_n = 0$. %Assuming $q(y|x)$ is realisable (which we do), $U_n$ behaves nicely ???insert some of those nice properties???.

...
\textbf{RLCT and likelihood vs temperature}. Let us assume that our statistical model is of the exponential form  (\ref{eq:gaussian_model_in_w}) and let $\mathcal D_n = \{ (x_i,y_i) \}_{i=1}^n$ be sampled from the true distribution. The negative log likelihood is
%\begin{equation}
$
...
Theoretical RLCTs are not known for modern neural networks where the weights number on the order of $n$. 
